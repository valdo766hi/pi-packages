...
import { SEARCH_PAGE_SIZE, searchSkills } from "./src/search.ts";
...
test("loader strips frontmatter and defines offsets against the stripped body", async () => {
	...
	const continued = await loadSkill(
		skill as never,
		0,
		undefined,
		2,
	...
	await assert.rejects(
		loadSkill(skill as never, 0, undefined, 4, 1, undefined, loaded.sourceRevision),
		assertCode("SKILL_OFFSET_INVALID"),
	);
	await assert.rejects(
		loadSkill(skill as never, 0, undefined, 1, 100, undefined, loaded.sourceRevision),
		assertCode("SKILL_OFFSET_INVALID"),
	);
});
...
async function createHarness(options: HarnessOptions): Promise<Harness> {
			...
			return [
				{
					...
					promptGuidelines: skillTool.promptGuidelines,
					sourceInfo: {
						path:
	...
	const registeredTool = tools.get("skill");
	...
	const harness: Harness = {
		...
		tool: registeredTool,
		searchTool: tools.get("skill_search"),
		commands,
		notifications,
		sent,
...
function catalogText(result: unknown): string {
...
test("extension registers the compact fixed tool contract", async () => {
	...
	assert.ok(harness.searchTool);
	assert.equal(harness.searchTool.name, "skill_search");
	assert.equal(
		harness.searchTool.description,
...
test("canonical discovery feeds safe prompt and frontmatter-free exact-name loading", async () => {
	...
	const transformed = (await harness.before()) as {
		systemPrompt: string;
		message?: { content: string };
	};
	const visible = catalogText(transformed);
	for (const skill of discovered.skills) {
		assert.ok(visible.includes(`<skill name="${skill.name}">`));
		assert.ok(visible.includes(skill.description));
		assert.ok(!visible.includes(skill.filePath));
	}
	assert.ok(!transformed.systemPrompt.includes(discovered.skills[0]?.filePath ?? "missing"));
...
test("bash fallback and skill-only mode publish the same safe catalog", async () => {
	...
	const bashPrompt = (await bashOnly.before()) as {
		systemPrompt: string;
		message?: { content: string };
	};
	assert.ok(catalogText(bashPrompt).includes("Alpha complete description"));
	assert.ok(!catalogText(bashPrompt).includes(alpha.filePath));
	...
	assert.ok(catalogText(appended).includes("Alpha complete description"));
	assert.ok(!catalogText(appended).includes(alpha.filePath));
});
...
test("skill-only restrictive policy sanitizes unexpected native sections", async () => {
	...
	assert.ok(ambiguous.systemPrompt.includes("custom"));
...
test("session reload applies changed policy before the next model turn", async () => {
	...
	const harness = await createHarness({ skills: [alpha] });
	await harness.sessionStart();
	const initial = (await harness.before()) as {
		systemPrompt: string;
		message?: { content: string };
	};
	assert.ok(catalogText(initial).includes("Alpha description"));
...
test("a failed snapshot rebuild blocks new skill operations", async () => {
	...
	const failed = (await harness.before("", "base host instructions")) as {
		systemPrompt: string;
	};
	assert.match(failed.systemPrompt, /SKILL_PROMPT_INTEGRATION_FAILED/u);
	assert.ok(failed.systemPrompt.includes("base host instructions"));
	await assert.rejects(
		harness.tool.execute(
			"after-failure",
			{ name: "alpha" },
		...
		assertCode("POLICY_INVALID"),
	);
	assert.ok(harness.searchTool);
	await assert.rejects(
		harness.searchTool.execute(
	...
	await harness.input("/lazy-skill alpha");
	await assert.rejects(
		harness.tool.execute(
			"after-command",
			{ name: "alpha" },
	...
	assert.ok(restored.content[0]?.text.includes("# Alpha"));
});
...
test("real-world skill keeps complete late routing text and reduces loaded payload", async () => {
	...
	const harness = await createHarness({ skills: discovered.skills });
	await harness.sessionStart();
	const prompt = (await harness.before()) as {
		systemPrompt: string;
		message?: { content: string };
	};
	assert.ok(
		catalogText(prompt).includes("investigate a real production incident"),
	);
	const result = await harness.tool.execute(
		"real-world",
		{ name: skill.name },
...
test("paginated skill bodies bind continuation to a source revision", async () => {
	const skill = await temporarySkill(
		"revision",
		"first line\nsecond line\nthird line\n",
	);
	const loaded = await loadSkill(skill as never, 0);
	await assert.rejects(
		loadSkill(skill as never, 0, undefined, 2, 1),
		assertCode("SKILL_SOURCE_CHANGED"),
	);
	await assert.rejects(
		loadSkill(
			skill as never,
	...
	assert.equal(continued.body, "second line\nthird line");
	assert.equal(continued.sourceRevision, loaded.sourceRevision);
});

test("skill_search browses every policy-visible skill and rejects stale cursors", async () => {
	const skills = [
		makeSkill("alpha", "Alpha description"),
		makeSkill("beta", "Beta description"),
		makeSkill("denied", "Denied secret"),
		makeSkill("hidden", "Explicit only", undefined, undefined, true),
	];
	const harness = await createHarness({ skills });
	await mkdir(join(harness.ctx.cwd, ".pi"), { recursive: true });
	await writeFile(
		...
		JSON.stringify({
			permission: {
				skill: { rules: [{ pattern: "denied", action: "deny" }] },
			},
		}),
	);
	await harness.sessionStart();
	await harness.before();
	assert.ok(harness.searchTool);
	const page = await harness.searchTool.execute(
		"search-1",
		{},
		undefined,
		undefined,
	...
	assert.ok(text.includes("Alpha description"));
	assert.ok(text.includes("Beta description"));
	assert.ok(!text.includes("Denied secret"));
	assert.ok(!text.includes("Explicit only"));
	const meta = JSON.parse(page.content[1]?.text ?? "{}") as {
		hasMore: boolean;
		mode: string;
	};
	assert.equal(meta.mode, "browse");
	...
	await assert.rejects(
		...
		assertCode("SKILL_SEARCH_CURSOR_STALE"),
	);
});

test("skill_search browse-all returns every eligible skill exactly once per snapshot", () => {
	const skills = Array.from({ length: SEARCH_PAGE_SIZE + 2 }, (_, index) =>
		makeSkill(
			`browse-${String(index).padStart(2, "0")}`,
			`Browse description ${index}`,
		),
	);
	...
	let pages = 0;
	do {
		const page = searchSkills(
			snapshot,
			cursor === undefined ? {} : { cursor },
		);
		pages += 1;
		for (const hit of page.skills) names.push(hit.name);
