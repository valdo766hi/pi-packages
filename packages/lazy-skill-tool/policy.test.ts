...
test("configuration defaults to adaptive routing, complete descriptions, and no sampling", () => {
	const result = readConfig({});
	assert.equal(result.config.routing, "adaptive");
	assert.equal(result.config.maxDescriptionCharacters, 0);
	assert.equal(result.config.resourceFileSampleLimit, 0);
	assert.equal(result.config.catalogTokenBudget, DEFAULT_CATALOG_TOKEN_BUDGET);
...
test("configuration parser accepts the documented shape and rejects ambiguity", () => {
	...
	assert.equal(parsed.catalogTokenBudget, 1024);
